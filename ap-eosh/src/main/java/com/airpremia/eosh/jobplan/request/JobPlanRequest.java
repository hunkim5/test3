...
public class JobPlanRequest {
    ...
    private List<ManageSupervisionEmployeeRequest> managerList;
    @Schema(description = "근로자")
    private List<JobEmployeeRequest> employeeList;
    @Schema(description = "근로자 보호구 확인")
    private List<JobEmployeeProtectGearCheckRequest> employeeProtectGearList;
    @Schema(description = "워키토키")
    private List<JobWalkieTalkieRequest> jobWalkieTalkieList;
    @Schema(description = "안전보건조치 요구사항")
    private List<SafetyHealthMeasuresRequirementsRequest> safetyRequirementList;
    @Schema(description = "밀폐 공간 작업 계획서 및 허가서")
    private ConfinedPlaceJobPlanPermissionRequest confinedPlacePermission;
    @Schema(description = "화기 작업 계획서 및 허가서")
    private FirearmsJobPlanPermissionRequest firearmsPermission;
    @Schema(description = "차량계 하역 운반 기계 작업 계획서")
    private WorldVehiclesLoadUnloadCarryMachineRequest vehiclesJobPlan;
}
